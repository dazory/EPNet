...
def pad(
        img: Tensor, padding: Union[int, List[int]], fill: Optional[Union[int, float]] = 0,
...
def denormalize(t_img):
    # t_img = t_img * 255.0

    # IMAGENET 형식으로 normalize 된 경우
    IMAGENET_MEAN, IMAGENET_STD = np.array([0.485, 0.456, 0.406]), np.array([0.229, 0.224, 0.225])
    denorm_img = np.clip(255.0 * (t_img * IMAGENET_STD + IMAGENET_MEAN), 0, 255)

    return denorm_img


def torch_to_opencv(t_img):
    cv_img = t_img.detach().cpu().numpy()  # tensor -> numpy
    cv_img = np.transpose(cv_img, (1, 2, 0))  # [C,H,W] -> [H,W,C]
    cv_img = denormalize(cv_img)  # *255 or IMAGENET denorm 방법
    print("cv_img.shape: ", cv_img.shape)
    cv_img = cv_img.astype(np.uint8)
    cv_img = cv2.cvtColor(cv_img, cv2.COLOR_BGR2RGB)  # RGB 채널
    # cv_img = cv_img.astype(np.uint8).copy()  # np.float32 -> np.uint8

    return cv_img

def random_aug(img, rand_num):
    if rand_num == 1: # gaussian_blurr
        ret_img = gaussian_blur(img, [3, 3], None)
    elif rand_num == 2:
