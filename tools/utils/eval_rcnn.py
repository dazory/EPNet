...
def eval_one_epoch_rpn(model, dataloader, epoch_id, result_dir, logger, wandb_logger, args):
    np.random.seed(1024)
...
def eval_one_epoch_rcnn(model, dataloader, epoch_id, result_dir, logger, wandb_logger, args):
    np.random.seed(1024)
...
def eval_one_epoch_joint(model, dataloader, epoch_id, result_dir, logger, wandb_logger, args):
    np.random.seed(666)
    MEAN_SIZE = torch.from_numpy(cfg.CLS_MEAN_SIZE[0]).cuda()
    mode = 'TEST' if args.test else 'EVAL'
...
def eval_one_epoch(model, dataloader, epoch_id, result_dir, logger, wandb_logger, args):
    if cfg.RPN.ENABLED and not cfg.RCNN.ENABLED:
        ret_dict = eval_one_epoch_rpn(model, dataloader, epoch_id, result_dir, logger, wandb_logger, args)
    elif not cfg.RPN.ENABLED and cfg.RCNN.ENABLED:
        ret_dict = eval_one_epoch_rcnn(model, dataloader, epoch_id, result_dir, logger, wandb_logger, args)
    elif cfg.RPN.ENABLED and cfg.RCNN.ENABLED:
        ret_dict = eval_one_epoch_joint(model, dataloader, epoch_id, result_dir, logger, wandb_logger, args)
    else:
...
def load_ckpt_based_on_args(model, logger, args):
...
def eval_single_ckpt(root_result_dir, args):
